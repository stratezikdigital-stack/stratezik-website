...
export function BlogAiSummarizeBar({ title, slug, className = '' }: BlogAiSummarizeBarProps) {
  ...
  return (
    <aside
      aria-label="Get this article summarized with AI"
      className={`mt-10 border border-ink/10 bg-cream-50/60 px-5 py-4 md:px-6 md:py-5 ${className}`}
    >
      <h2 className="text-sm font-semibold text-ink md:text-base">Get it Summarized with AI</h2>
      <p className="mt-2 text-sm text-ink-600 leading-relaxed">
        We copy a ready-made prompt to your clipboard, then open your assistant.{' '}
        <span className="font-semibold text-ink">
          If the chat is empty, paste with <span className="font-mono">⌘V</span> /{' '}
