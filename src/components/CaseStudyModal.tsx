...
import { pauseLenis, resumeLenis } from '../three/world/lenisRef'
...
const CaseStudyModal = ({ open, mode, angle, matchNum, onClose }: CaseStudyModalProps) => {
  const closeRef = useRef<HTMLButtonElement>(null)
  const sheetRef = useRef<HTMLDivElement>(null)

  // Lock page scroll while open. We pause Lenis (which owns wheel events
  // globally) AND set body overflow as a belt-and-braces measure so even
  // non-Lenis touch scroll on mobile stays trapped behind the modal.
  useEffect(() => {
    if (!open) return
  ...
  // Reset modal scroll position whenever a different case is opened
  useEffect(() => {
    if (open) sheetRef.current?.scrollTo({ top: 0 })
  }, [open, angle, matchNum])

  const payload = useMemo(() => getCaseStudyPayload(mode), [mode])
  const view = useMemo(() => getCaseStudyView(angle, mode), [angle, mode])

  // Filter scorecard rows to only those the view wants to surface, in order.
  const scorecardRows = useMemo(() => {
    const lookup = new Map(payload.scorecard.map((r) => [r.metric, r]))
    return view.show.scorecardMetrics
      .map((m) => lookup.get(m))
      .filter((r): r is NonNullable<typeof r> => Boolean(r))
  }, [payload.scorecard, view.show.scorecardMetrics])

  const phases = view.show.phaseIdxs.map((i) => payload.phases[i]).filter(Boolean)
  const timeline = view.show.timelineIdxs.map((i) => payload.timeline[i]).filter(Boolean)
  const approach = view.show.approachIdxs.map((i) => payload.approach[i]).filter(Boolean)
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative my-0 md:my-8 mx-0 md:mx-6 w-full max-w-[1180px] bg-cream text-ink overflow-y-auto overflow-x-hidden overscroll-contain shadow-[0_40px_80px_-20px_rgba(0,0,0,0.45)] border-y md:border border-ink/15"
          >
            ...
            <header className="relative bg-ink text-cream px-6 md:px-16 pt-16 pb-12 md:pt-24 md:pb-16 overflow-hidden">
              ...
              <div className="relative max-w-4xl">
                ...
                <h1 className="font-display text-[clamp(2.25rem,6vw,4.5rem)] leading-[1.02] tracking-[-0.025em] text-cream">
                  {view.hero.headline.lead}
                  <br />
                  <span className="text-oxblood-400">{view.hero.headline.accent}</span>{' '}
                  <span className="italic font-light">{view.hero.headline.tail}</span>
                </h1>
              ...
              <div className="relative mt-10 md:mt-14 grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-2 max-w-4xl">
                {view.hero.stats.map((s) => (
                  <div key={s.lbl} className="border-l border-cream/20 pl-4 md:pl-5">
                    ...
                    <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.18em] text-cream/60">
                      {s.lbl}
                    </div>
                  </div>
                ))}
            ...
            {phases.length > 0 && (
              <section className="px-6 md:px-16 py-14 md:py-20 border-b border-ink/15">
                <div className="font-mono text-[11px] uppercase tracking-[0.22em] text-oxblood mb-3">
                  {phases.length === 1 ? 'The Decisive Phase' : 'The Journey'}
                </div>
                <h2 className="font-display text-3xl md:text-4xl text-ink leading-[1.05] tracking-[-0.025em]">
                  {phases.length === 1
                    ? phases[0].title
            ...
            {view.show.gbpCallout && (
              <section className="px-6 md:px-16 py-14 md:py-20 border-b border-ink/15 bg-ink text-cream relative overflow-hidden">
                ...
                <div className="relative max-w-3xl">
                  ...
                  <p className="mt-5 text-cream/80 leading-relaxed text-base md:text-lg">
